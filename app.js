import { AppConfig, MathUtil, ZzzMath, StandardCalculator, AnomalyCalculator, RuptureCalculator, Preview, StatMeta, MarginalAnalyzer } from "./zzz_logic.js";
  ...
  class I18n {
    ...
    static STRINGS = {
      ...
      ko: {
        ...
        "label.resReductionPct": "모든 속성 저항 (%)",
  ...
  class App {
    ...
    refresh() {
      ...
      const marginal = MarginalAnalyzer.compute(i, {
        ...
        getLabel: (meta) => I18n.t(meta.labelKey || meta.label),
        effectiveDisplay: (inputs, key, rawValue) => {
          if (key === "critRatePct") {
          ...
          if (key === "defReductionPct" || key === "defIgnorePct") {
            const defRed = (key === "defReductionPct") ? rawValue : Number(inputs.enemy.defReductionPct) || 0;
            const defIgn = (key === "defIgnorePct") ? rawValue : Number(inputs.agent.defIgnorePct) || 0;
